...
try:
    ...
    st.warning("⚠️ 현재 테스트 환경(my_assets_ex.csv) 모드입니다.")
    ...
    def make_display_table(target_df, cols):
        ...
        return res.style.format({
            ...
            '현재가': lambda x: safe_format(x), '평가액': '{:,.0f}', '수익률': '{:.2f}%'
        })

    # --- TAB 1: 상세 (기능 6 차트 포함) ---
    with tab1:
        sort_ref = asset_df.groupby('종목명')['평가금액'].sum().sort_values(ascending=False).index.tolist()
        selected_stock = st.selectbox("📂 종목", sort_ref)
        ...
        st.dataframe(make_display_table(target_df.sort_values(by='평가금액', ascending=False), ['계좌명', '보유수량', '매수평단', '현재가', '평가금액', '수익률']), use_container_width=True, hide_index=True)
    ...
    with tab3:
        for acc in ["연금저축(키움)", "IRP(미래)", "연금저축(미래)", "경성IRP(삼성)", "중개형ISA(키움)"]:
            if acc in asset_df['계좌명'].unique():
                ...
                st.dataframe(make_display_table(acc_assets.sort_values(by='평가금액', ascending=False), ['약식종목명', '보유수량', '매수평단', '현재가', '평가금액', '수익률']), use_container_width=True, hide_index=True)
