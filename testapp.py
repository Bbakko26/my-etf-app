...
try:
    ...
    with st.spinner('실시간 시세 동기화 중...'):
        for code in unique_codes:
            ...
            try:
                ...
                price_map[code] = p
                chg_map[code] = ((p - hist['Close'].iloc[-2]) / hist['Close'].iloc[-2] * 100)
    ...
    with tabs[4]:
        ...
        if current_fx < 1300:
            advice, target_ratio = "환율 저점: **환노출 비중 80% 이상** 추천", {"노출": 80, "헤지": 20}
        elif current_fx > 1400:
            advice, target_ratio = "환율 고점: **환헤지 비중 70% 이상** 추천", {"노출": 30, "헤지": 70}
        else:
            advice, target_ratio = "중립 구간: **환노출 50 : 환헤지 50** 추천", {"노출": 50, "헤지": 50}
        st.info(advice)

        st.subheader("📊 주요 자산군 환율 대응 현황 (세액공제 계좌)")
        for target_group in ["S&P500", "나스닥100"]:
            fx_sub = asset_df[(asset_df['자산군'] == target_group) & (asset_df['계좌카테고리'].isin(["세액공제 O", "세액공제 X"])) & (asset_df['보유수량'] > 0)].copy()
            if not fx_sub.empty:
