...
try:
    ...
    # --- [2. 종목 선택 드롭다운] ---
    selected_stock = st.selectbox("📂 상세 조회 종목 선택", df['종목명'].unique())
    target_df = df[df['종목명'] == selected_stock].copy()
    target_df['수익금'] = target_df['평가금액'] - target_df['매수금액']
    target_df['수익률'] = (target_df['수익금'] / target_df['매수금액']) * 100
    ...
    # --- [3. 상세 정보 탭] ---
    tab1, tab2, tab3 = st.tabs(["📈 차트 & 상세", "🍩 자산 비중", "🏦 전체 계좌"])
    ...
    with tab1:
        ...
        st.table(summary_data.style.format({'보유수량': '{:,.0f}', '평단가': '{:,.0f}', '현재가': '{:,.0f}', '평가금액': '{:,.0f}', '수익률': '{:.2f}%'}))
        
        st.write("계좌별 내역")
        st.table(target_df[['계좌명', '보유수량', '매수평단', '현재가', '평가금액', '수익률']].style.format({
    ...
    with tab2:
        ...
        # 📍 [수정] 종목별 합산 요약 테이블 항목 대폭 보강
        st.subheader("📊 종목별 통합 현황")
        sum_df = df.groupby(['종목명']).agg({
            '보유수량': 'sum',
            '매수금액': 'sum',
            '평가금액': 'sum'
        }).reset_index()
        
        # 현재가 매핑 (각 종목의 첫 번째 현재가 사용)
        price_lookup = df.drop_duplicates('종목명')[['종목명', '현재가']]
