...
def make_dual_donut(
    ...
    if center_title or center_value:
        center_parts = []
        if center_title:
            center_parts.append(f"<span style='font-size:12px;color:#94a3b8'>{center_title}</span>")
        if center_value:
            center_parts.append(f"<span style='font-size:16px;font-weight:700'>{center_value}</span>")
        fig.add_annotation(
            x=0.5,
            y=0.5,
            text="<br>".join(center_parts),
            showarrow=False,
            xanchor="center",
            yanchor="middle",
            align="center",
...
def get_market_history(symbol, fallback=None, period="8mo"):
    ...
    for sym in candidates:
        try:
            hist = fdr.DataReader(sym)
            if hist is not None and not hist.empty:
                hist = hist.copy()
                hist.index = pd.to_datetime(hist.index)
                if "Close" in hist.columns:
                    return hist.tail(180)
        except Exception:
            pass
...
def market_snapshot(symbol, fallback=None):
    ...
    if hist is None or hist.empty or "Close" not in hist.columns:
        return {"current": None, "daily": None, "monthly": None, "hist": pd.DataFrame(), "has_data": False}

    close = pd.to_numeric(hist["Close"], errors="coerce").dropna()
    if len(close) < 2:
        return {"current": None, "daily": None, "monthly": None, "hist": hist.tail(120), "has_data": False}
    ...
    return {"current": current, "daily": daily, "monthly": monthly, "hist": hist.tail(120), "has_data": True}
...
def make_monitor_chart(hist_df, title=""):
    ...
    if hist_df is not None and not hist_df.empty and "Close" in hist_df.columns:
        fig.add_trace(go.Scatter(x=hist_df.index, y=hist_df["Close"], mode="lines"))
    fig.update_layout(title=None, height=280, margin=dict(l=10, r=10, t=10, b=10), showlegend=False)
    return fig
...
try:
    ...
    # 5. 환율관리
    with tabs[4]:
        open_card(f"🌎 실시간 환율: {current_fx:,.2f}원", "현재 환율 구간, 달러 기준 기초자산 흐름, 환노출/환헤지 비중")
        ...
        st.markdown("#### 달러 기준 기초자산 모니터링")
        st.caption("원화 환율 효과를 제외하고, 기초자산 자체의 달러 기준 움직임을 확인합니다.")
        ...
        for idx, item in enumerate(snapshots):
            with mon_cols[idx % 2]:
                daily_html = pct_badge_html(item["daily"]) if item.get("has_data") else "<span style='color:#94a3b8'>데이터 없음</span>"
                monthly_html = pct_badge_html(item["monthly"]) if item.get("has_data") else "<span style='color:#94a3b8'>데이터 없음</span>"
                source_text = f"{item['symbol']} / {item['fallback']}" if item.get("fallback") else item["symbol"]
                st.markdown(
                    f"""
                    <div class="app-card" style="margin:0 0 12px 0;">
                        <div class="app-card-title">{item["name"]}</div>
                        <div class="app-card-caption">현재값 {value_text(item["current"], item["unit"])} · 소스 {source_text}</div>
        ...
        selected_monitor = st.selectbox("모니터링 자산 선택", monitor_names, key="monitor_select_in_fx")
        selected_item = next(item for item in snapshots if item["name"] == selected_monitor)

        if selected_item.get("has_data") and selected_item["hist"] is not None and not selected_item["hist"].empty:
            st.plotly_chart(
            ...
            st.info("선택한 자산의 시계열 데이터를 불러오지 못했습니다. 심볼 또는 네트워크 상태를 확인해 주세요.")

        monitor_df = pd.DataFrame([
            {
                "자산": item["name"],
                "현재값": value_text(item["current"], item["unit"]),
                "전일 대비": f"{item['daily']:.2f}%" if item["daily"] is not None else "데이터 없음",
                "1개월": f"{item['monthly']:.2f}%" if item["monthly"] is not None else "데이터 없음",
                "기본 심볼": item["symbol"],
                "대체 심볼": item["fallback"],
            }
        ...
        st.markdown("#### 환율 도넛 비교")
        st.caption("각 계좌 카테고리별 해외 주식형 자산의 현재 환노출/환헤지 비중과 권장 비율을 비교합니다.")
        ...
        for cat_name in CATEGORY_ORDER:
            for target_asset in fx_config.get(cat_name, []):
                ...
                fx_items.append((cat_name, target_asset, fx_grp))

        for i in range(0, len(fx_items), 2):
            cols = st.columns(2)
            for col, item in zip(cols, fx_items[i:i+2]):
                cat_name, target_asset, fx_grp = item
                with col:
                    open_card(f"{cat_name} · {display_asset_group(target_asset)}", "현재 비중 vs 권장 환비중")
