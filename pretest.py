...
try:
    ...
    with tabs[4]:
        ...
        guide_data = [
            ...
            {"환율 구간": "1,400 ~ 1,450원", "단계": "주의", "신규 매수 액션": "환헤지(H) 70% : 환노출 30%", "신규 적립금으로 환헤지 비중을 우선 채움"},
            {"환율 구간": "1,330 ~ 1,400원", "단계": "중립", "신규 매수 액션": "환헤지(H) 30% : 환노출 70%", "비중 이탈이 없다면 기존 포지션 유지"},
            {"환율 구간": "1,250 ~ 1,330원", "단계": "안정", "신규 매수 액션": "100% 환노출 종목 매수", "환헤지(H) 종목을 환노출로 교체 시작"},
            {"환율 구간": "1,250원 미만", "단계": "기회", "100% 환노출 + 적극 매수", "모든 (H) 종목을 환노출로 전환 완료"}
        ]
