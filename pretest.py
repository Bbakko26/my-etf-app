...
def create_double_donut(labels, current_vals, target_vals, title, key_suffix=""):
    # 환율 관리용 색상 고정 (노출: 빨강계열, 헤지: 파랑계열)
    color_map = {'환노출': '#EF553B', '환헤지': '#636EFA'}
    # 자산군용 색상은 파스텔톤 자동 할당
    if labels[0] not in color_map:
        colors = px.colors.qualitative.Pastel
        color_map = {label: colors[i % len(colors)] for i, label in enumerate(labels)}

    fig = go.Figure()

    # 1. 바깥쪽 얇은 띠 (목표 비중 - 가이드라인)
    fig.add_trace(go.Pie(
        labels=labels, values=target_vals,
        hole=0.88, sort=False, direction='clockwise',
...
try:
    ...
    with tabs[0]:
        ...
        st.divider()
        sel_name = st.selectbox("종목 상세 선택", sum_df['종목명'].unique())
        ...
        if not detail_df.empty and detail_df['종목코드'].iloc[0].upper() != "CASH":
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"dt_{sel_name}")
    ...
    with tabs[1]:
        ...
        st.plotly_chart(create_double_donut(grp_df['자산군'].tolist(), grp_df['비중'].tolist(), grp_df['목표'].tolist(), "전체 자산 배분 (안쪽:현재 vs 바깥띠:목표)"), use_container_width=True, key="main_double")
    ...
    # 5. 환율관리 (📍 이중 도넛 적용)
    with tabs[4]:
        st.subheader(f"🌎 실시간 환율: {current_fx:,.0f}원")
        # 권장 비율 설정
        ...
        for cat, targets in fx_config.items():
            ...
            for i, asset in enumerate(targets):
                ...
                if not f_df.empty:
                    ...
                    total_asset_eval = sum(curr_vals)
                    curr_pcts = [(v/total_asset_eval)*100 if total_asset_eval > 0 else 0 for v in curr_vals]
                    target_pcts = [t_fx['노출'], t_fx['헤지']]
                    
                    with cols[i]:
                        st.plotly_chart(create_double_donut(labels, curr_pcts, target_pcts, f"📊 {asset} 대응 현황"), use_container_width=True, key=f"fx_double_{cat}_{asset}")
