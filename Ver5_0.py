...
def _resolve_colors(names, color_map):
    """색상맵에서 이름별 색상을 추출; 없으면 ASSET_COLOR_MAP → 기본 팔레트 순으로 fallback"""
    FALLBACK_PALETTE = [
        "#6366f1","#8b5cf6","#f59e0b","#10b981","#06b6d4",
        "#f97316","#3b82f6","#ec4899","#14b8a6","#64748b",
    ]
    result = []
    fallback_idx = 0
    for name in names:
        if color_map and name in color_map:
            result.append(color_map[name])
        elif name in ASSET_COLOR_MAP:
            result.append(ASSET_COLOR_MAP[name])
        else:
            result.append(FALLBACK_PALETTE[fallback_idx % len(FALLBACK_PALETTE)])
            fallback_idx += 1
    return result
...
def make_dual_donut(
    ...
    current_map_vals = current_df.groupby(current_name_col)[current_value_col].sum().to_dict()
    current_vals = [float(current_map_vals.get(name, 0)) for name in names]
    ...
    current_colors = _resolve_colors(names, current_color_map)
    target_colors  = _resolve_colors(names, target_color_map)
    ...
    fig.add_trace(
        go.Pie(
            ...
            marker=dict(
                ...
                line=dict(color="rgba(15,23,42,0.85)", width=2),
            ),
    ...
    fig.add_trace(
        go.Pie(
            ...
            marker=dict(
                ...
                line=dict(color="rgba(15,23,42,0.6)", width=1.5),
            ),
    ...
    if center_title or center_value:
        ...
        if center_title:
            parts.append(
                f"<span style='font-size:11px;color:#94a3b8;letter-spacing:0.04em'>"
                f"{center_title}</span>"
            )
        if center_value:
            parts.append(
                f"<span style='font-size:17px;font-weight:800;color:#f1f5f9'>"
                f"{center_value}</span>"
            )
        fig.add_annotation(
            x=0.5, y=0.5,
...
MARKET_MONITORS = [
    {"name": "S&P 500", "symbol": "^GSPC", "fallback": "SPY", "unit": "index"},
    {"name": "Nasdaq 100", "symbol": "^NDX", "fallback": "QQQ", "unit": "index"},
    {"name": "미국채 10년물", "symbol": "^TNX", "fallback": "IEF", "unit": "yield"},
    {"name": "국제 금값", "symbol": "GC=F", "fallback": "GLD", "unit": "usd"},
]

@st.cache_data(ttl=900, show_spinner=False)
def get_market_history(symbol, fallback=None, period="8mo"):
    if yf is None:
        return pd.DataFrame()
    candidates = [symbol]
    ...
    for sym in candidates:
        try:
            hist = yf.download(sym, period=period, interval="1d", auto_adjust=False, progress=False)
...
def market_snapshot(symbol, fallback=None):
    hist = get_market_history(symbol, fallback=fallback)
    if hist is None or hist.empty or "Close" not in hist.columns:
        return {"current": None, "daily": None, "monthly": None, "hist": pd.DataFrame()}
    close = hist["Close"].dropna()
    if len(close) < 2:
        return {"current": None, "daily": None, "monthly": None, "hist": hist.tail(120)}
    current = float(close.iloc[-1])
    prev = float(close.iloc[-2])
    daily = ((current - prev) / prev * 100.0) if prev else None
    monthly = None
    if len(close) >= 22:
        month_prev = float(close.iloc[-22])
        monthly = ((current - month_prev) / month_prev * 100.0) if month_prev else None
    return {"current": current, "daily": daily, "monthly": monthly, "hist": hist.tail(120)}

def pct_badge_html(value):
    if value is None:
        return "<span style='color:#94a3b8'>-</span>"
    color = "#ef4444" if value > 0 else "#60a5fa"
    sign = "+" if value > 0 else ""
    return f"<span style='color:{color}; font-weight:700'>{sign}{value:.2f}%</span>"

def value_text(value, unit="index"):
    if value is None:
        return "-"
    if unit == "yield":
        return f"{value:.2f}"
...
def make_monitor_chart(hist_df, title=""):
    fig = go.Figure()
    if hist_df is not None and not hist_df.empty and "Close" in hist_df.columns:
        fig.add_trace(go.Scatter(x=hist_df.index, y=hist_df["Close"], mode="lines", name=title))
    fig.update_layout(
        title=None,
        height=280,
...
try:
    ...
    tab_labels = ["📊 종목 상세", "🍩 전체 비중", "🏦 카테고리", "💼 계좌별", "🌎 환율관리", "⚖️ 리밸런싱", "📅 월별 적립기록"]
    tabs = st.tabs(tab_labels)

    with tabs[0]: st.session_state.active_view = "📊 종목 상세"
    with tabs[1]: st.session_state.active_view = "🍩 전체 비중"
    with tabs[2]: st.session_state.active_view = "🏦 카테고리 분석"
    with tabs[3]: st.session_state.active_view = "💼 계좌별"
    with tabs[4]: st.session_state.active_view = "🌎 환율관리"
    with tabs[5]: st.session_state.active_view = "⚖️ 리밸런싱"
    with tabs[6]: st.session_state.active_view = "📅 월별 적립기록"

    # 탭 클릭 감지용 (session_state 동기화)
    if st.session_state.active_view == "📊 종목 상세":
    ...
    # 2. 전체 비중
    if st.session_state.active_view == "🍩 전체 비중":
        ...
        overall_gap = grp_df["차이"].abs().sum() / 2 if not grp_df.empty else 0
        st.plotly_chart(
            make_dual_donut(
                grp_df,
                "비중",
                ...
                DONUT_TARGET_DOMAIN,
                center_title="총자산",
                center_value=format_krw_short(total_eval),
            ),
            use_container_width=True,
    ...
    # 3. 카테고리 분석
    if st.session_state.active_view == "🏦 카테고리 분석":
        for cat_name in CATEGORY_ORDER:
            ...
            category_target_map = {k: v * 100 for k, v in CATEGORY_TARGETS[cat_name].items()}
            category_gap = 0.0
            if not cat_grp.empty:
                category_gap = cat_grp["차이"].abs().sum() / 2
            st.plotly_chart(
                make_dual_donut(
                    cat_grp,
                    "비중",
                    ...
                    DONUT_CURRENT_DOMAIN,
                    DONUT_TARGET_DOMAIN,
                    center_title=cat_name,
                    center_value=format_krw_short(cat_total),
                ),
                use_container_width=True,
    ...
    # 4. 계좌별
    if st.session_state.active_view == "💼 계좌별":
    ...
    # 5. 환율관리
    if st.session_state.active_view == "🌎 환율관리":
        ...
        for cat_name in CATEGORY_ORDER:
            ...
            for target_asset in fx_config.get(cat_name, []):
                ...
                fx_center = f"노출 {fx_target['환노출']:.0f}% / 헤지 {fx_target['환헤지']:.0f}%"
                st.plotly_chart(
                    make_dual_donut(
                        fx_grp,
                        "비중",
                        ...
                        center_title=display_asset_group(target_asset),
                        center_value=fx_center,
                    ),
                    use_container_width=True,
    ...
    # 6. 리밸런싱
    if st.session_state.active_view == "⚖️ 리밸런싱":
    ...
    if st.session_state.active_view == "📈 달러기준 모니터링":
        open_card("📈 달러기준 모니터링", "환율 효과와 분리해서 달러 기준 기초자산 움직임을 확인")
        snapshots = []
        for item in MARKET_MONITORS:
            snap = market_snapshot(item["symbol"], fallback=item["fallback"])
        ...
        for idx, item in enumerate(snapshots):
            with top_cols[idx % 2]:
                st.markdown(
                    f'''
                    <div class="app-card" style="margin:0 0 12px 0;">
                        <div class="app-card-title">{item["name"]}</div>
                        <div class="app-card-caption">현재값 {value_text(item["current"], item["unit"])}</div>
                        <div style="display:flex; gap:10px; align-items:center;">
                            <div style="font-size:0.86rem;color:#94a3b8;">전일</div>
                            <div>{pct_badge_html(item["daily"])}</div>
                            <div style="font-size:0.86rem;color:#94a3b8; margin-left:8px;">1개월</div>
                            <div>{pct_badge_html(item["monthly"])}</div>
                        </div>
                    </div>
                    ''',
                    unsafe_allow_html=True,
        ...
        monitor_names = [item["name"] for item in snapshots]
        selected_monitor = st.selectbox("모니터링 자산 선택", monitor_names, key="monitor_select")
        selected_item = next(item for item in snapshots if item["name"] == selected_monitor)

        st.plotly_chart(
            make_monitor_chart(selected_item["hist"], title=selected_item["name"]),
            use_container_width=True,
            key=f"monitor_chart_{selected_item['name']}",
            config={"displayModeBar": False, "scrollZoom": False},
        )
        ...
        monitor_df = pd.DataFrame([
            {
                "자산": item["name"],
                "현재값": value_text(item["current"], item["unit"]),
                "전일 대비": f"{item['daily']:.2f}%" if item["daily"] is not None else "-",
                "1개월": f"{item['monthly']:.2f}%" if item["monthly"] is not None else "-",
                "기본 심볼": item["symbol"],
                "대체 심볼": item["fallback"],
            }
    ...
    # 7. 월별 적립기록
    if st.session_state.active_view == "📅 월별 적립기록":
        ACCUM_FILE = "monthly_accumulation.csv"
        ACCUM_COLS = ["연월", "계좌카테고리", "계좌명", "자산군", "약식종목명", "매수금액", "매수수량", "메모"]

        def load_accum():
            try:
                df = pd.read_csv(ACCUM_FILE, encoding="utf-8-sig", dtype={"연월": str})
            except FileNotFoundError:
                df = pd.DataFrame(columns=ACCUM_COLS)
            except Exception:
                try:
                    df = pd.read_csv(ACCUM_FILE, encoding="cp949", dtype={"연월": str})
                except Exception:
                    df = pd.DataFrame(columns=ACCUM_COLS)
            for col in ACCUM_COLS:
                if col not in df.columns:
                    df[col] = "" if col not in {"매수금액", "매수수량"} else 0
            df["매수금액"] = pd.to_numeric(df["매수금액"], errors="coerce").fillna(0)
            df["매수수량"] = pd.to_numeric(df["매수수량"], errors="coerce").fillna(0)
            return df

        def save_accum(df):
            df.to_csv(ACCUM_FILE, index=False, encoding="utf-8-sig")

        accum_df = load_accum()

        # ── 상단 요약 메트릭 ────────────────────────────────────────
        open_card("📅 월별 적립/매수 기록", "월별 적립 내역을 입력하고 추이를 확인하세요")

        if not accum_df.empty and accum_df["매수금액"].sum() > 0:
            total_accum = float(accum_df["매수금액"].sum())
            latest_ym = accum_df["연월"].dropna().max()
            latest_month_amt = float(accum_df[accum_df["연월"] == latest_ym]["매수금액"].sum())

            m1, m2, m3 = st.columns(3)
            m1.metric("누적 적립 총액", f"{total_accum:,.0f}원")
            m2.metric("최근 월 적립액", f"{latest_month_amt:,.0f}원", delta=latest_ym)
            m3.metric("적립 기록 건수", f"{len(accum_df):,}건")

        close_card()

        # ── 신규 기록 입력 ──────────────────────────────────────────
        open_card("➕ 새 적립 기록 입력", "이번 달 매수 내역을 추가하세요")

        category_options = CATEGORY_ORDER + ["기타"]
        asset_options = ["S&P500", "나스닥100", "다우존스", "금", "미국채", "금리", "현금", "기타"]

        col_a, col_b = st.columns(2)
        with col_a:
            inp_ym = st.text_input("연월 (예: 2025-04)", value=pd.Timestamp.now().strftime("%Y-%m"), key="inp_ym")
            inp_cat = st.selectbox("계좌카테고리", category_options, key="inp_cat")
            inp_account = st.text_input("계좌명", key="inp_account")
            inp_asset = st.selectbox("자산군", asset_options, key="inp_asset")
        with col_b:
            inp_ticker = st.text_input("약식종목명 (예: S&P500 노출)", key="inp_ticker")
            inp_amt = st.number_input("매수금액 (원)", min_value=0, step=10000, key="inp_amt")
            inp_qty = st.number_input("매수수량 (주)", min_value=0.0, step=0.01, format="%.2f", key="inp_qty")
            inp_memo = st.text_input("메모 (선택)", key="inp_memo")

        if st.button("✅ 기록 추가", use_container_width=True):
            if not inp_ym or not inp_account:
                st.warning("연월과 계좌명은 필수 입력 항목입니다.")
            else:
                new_row = {
                    "연월": inp_ym.strip(),
                    "계좌카테고리": inp_cat,
                    "계좌명": inp_account.strip(),
                    "자산군": inp_asset,
                    "약식종목명": inp_ticker.strip(),
                    "매수금액": float(inp_amt),
                    "매수수량": float(inp_qty),
                    "메모": inp_memo.strip(),
                }
                accum_df = pd.concat([accum_df, pd.DataFrame([new_row])], ignore_index=True)
                save_accum(accum_df)
                st.success(f"✅ {inp_ym} | {inp_account} | {inp_asset} 기록이 추가되었습니다.")
                st.rerun()
        ...
        # ── 월별 적립 추이 차트 ─────────────────────────────────────
        if not accum_df.empty and accum_df["매수금액"].sum() > 0:
            open_card("📈 월별 적립 추이", "월별 총 매수금액 추이")

            monthly_sum = (
                accum_df.groupby("연월")["매수금액"]
                .sum()
                .reset_index()
                .sort_values("연월")
            )
            ...
            # 누적합 계산
            monthly_sum["누적금액"] = monthly_sum["매수금액"].cumsum()
            ...
            fig_accum.add_trace(go.Bar(
                ...
                marker_color="#6366f1",
                opacity=0.85,
                hovertemplate="<b>%{x}</b><br>적립액: %{y:,.0f}원<extra></extra>",
            ))
            ...
            fig_accum.update_layout(
                ...
                xaxis=dict(tickfont=dict(color="#94a3b8"), gridcolor="rgba(148,163,184,0.08)"),
                yaxis=dict(title="월 적립액(원)", tickfont=dict(color="#94a3b8"),
                           gridcolor="rgba(148,163,184,0.08)", tickformat=",.0f"),
                yaxis2=dict(title="누적(원)", overlaying="y", side="right",
                            tickfont=dict(color="#f59e0b"), tickformat=",.0f", showgrid=False),
                barmode="overlay",
            )
            st.plotly_chart(fig_accum, use_container_width=True,
                            config={"displayModeBar": False}, key="accum_bar_chart")

            # 자산군별 파이 차트
            asset_grp = accum_df.groupby("자산군")["매수금액"].sum().reset_index()
            asset_grp = asset_grp[asset_grp["매수금액"] > 0]
            if not asset_grp.empty:
                st.markdown("#### 자산군별 누적 적립 비중")
                asset_colors = [ASSET_COLOR_MAP.get(a, "#64748b") for a in asset_grp["자산군"]]
                fig_pie = go.Figure(go.Pie(
                    labels=asset_grp["자산군"],
                    values=asset_grp["매수금액"],
                    hole=0.52,
                    marker=dict(colors=asset_colors, line=dict(color="rgba(15,23,42,0.85)", width=2)),
                    textinfo="label+percent",
                    textfont=dict(size=11, color="#f1f5f9"),
                ...
                st.plotly_chart(fig_pie, use_container_width=True,
                                config={"displayModeBar": False}, key="accum_pie_chart")

            close_card()

            # ── 기록 테이블 및 삭제 ────────────────────────────────
            open_card("📋 전체 적립 기록", "기존 기록을 확인하고 필요 시 삭제하세요")

            filter_col1, filter_col2 = st.columns(2)
            with filter_col1:
                ym_options = ["전체"] + sorted(accum_df["연월"].dropna().unique().tolist(), reverse=True)
                filter_ym = st.selectbox("연월 필터", ym_options, key="filter_ym")
            with filter_col2:
                cat_options = ["전체"] + sorted(accum_df["계좌카테고리"].dropna().unique().tolist())
                filter_cat = st.selectbox("카테고리 필터", cat_options, key="filter_cat")

            filtered = accum_df.copy()
            if filter_ym != "전체":
                filtered = filtered[filtered["연월"] == filter_ym]
            if filter_cat != "전체":
                filtered = filtered[filtered["계좌카테고리"] == filter_cat]

            filtered = filtered.sort_values(["연월", "계좌카테고리", "계좌명"], ascending=[False, True, True])
            st.dataframe(
                filtered[ACCUM_COLS].style.format({"매수금액": "{:,.0f}", "매수수량": "{:,.2f}"}),
                use_container_width=True,
                hide_index=True,
            )

            st.caption(f"필터 결과: {len(filtered)}건 / 합계: {filtered['매수금액'].sum():,.0f}원")

            with st.expander("🗑️ 기록 삭제 (주의)"):
                st.warning("아래에서 삭제할 행의 인덱스(0부터 시작)를 입력하세요. 원본 데이터 기준입니다.")
                del_idx = st.number_input("삭제할 행 번호 (원본 인덱스)", min_value=0,
                                          max_value=max(len(accum_df)-1, 0), step=1, key="del_idx")
                if st.button("해당 행 삭제", key="del_btn"):
                    accum_df = accum_df.drop(index=del_idx).reset_index(drop=True)
                    save_accum(accum_df)
                    st.success("삭제 완료.")
                    st.rerun()
            ...
            st.info("📭 아직 적립 기록이 없습니다. 위 입력 폼으로 첫 번째 기록을 추가해 보세요!")

        # CSV 내보내기
        if not accum_df.empty:
            csv_bytes = accum_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
            st.download_button(
                "⬇️ 적립 기록 CSV 다운로드",
                data=csv_bytes,
                file_name="monthly_accumulation.csv",
                mime="text/csv",
                use_container_width=True,
            )
